...
__global__ void repeat_kv(T* dst, const T* src, const int kv_head_num, const int repeat_num, const int size_per_head, const int token_num)
{
    for (int id = blockIdx.x * blockDim.x + threadIdx.x; id < kv_head_num * token_num * size_per_head; id += blockDim.x * gridDim.x) {
        ...
        for (int r = 0; r < repeat_num; r++) {
            // dst[0] = float(repeat_num);
            dst[r] = float(3 * repeat_num * kv_head_num * size_per_head * token_id + repeat_num * size_per_head * head_id + size_per_head * r + inner_id);
            // dst[0] = src[0];
            // dst[3 * repeat_num * kv_head_num * size_per_head * token_id + head_id * size_per_head * r + inner_id] = src[(repeat_num + 2) * kv_head_num * size_per_head * token_num + head_id * size_per_head * r + inner_id];
