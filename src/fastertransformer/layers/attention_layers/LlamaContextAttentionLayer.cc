                                          ...
                                          const AttentionWeight<T>* attention_weights)
{
    ...
    if (use_sparse) {
        ...
        cudaMemcpy(qkv_buf, qkv_buf_, sizeof(T) * st, cudaMemcpyDeviceToHost);
        printf("cudaMemcpy\n");
        for (int i=0; i<st; i++) {
            printf("%f ", double(qkv_buf[i]));
            if (i % 500 == 499 ) {
...
void LlamaContextAttentionLayer<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool allocate_qk_buf)
{
    ...
    size_t local_qkv_size = local_hidden_units_ + 2 * local_kv_head_num_ * size_per_head_;
    qkv_buf_tmp_ = (T*)allocator_->reMalloc(qkv_buf_, type_size * batch_size * seq_len * local_qkv_size, true);
